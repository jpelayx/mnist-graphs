...
class SuperPixelGraphCIFAR100(InMemoryDataset):
    def __init__(self, 
                 root=None, 
                 n_segments= 75,  
                 compactness = 0.1, 
        ...
        self.root = './cifar100/' + get_ds_name(self.n_segments, self.compactness, self.features, self.train) if root is None else root
    ...
    def loadCIFAR100(self):
        ...
        cifar100 = datasets.CIFAR100(cifar100_root, train=self.train, download=True, transform=T.ToTensor())
        img_total = cifar100.data.shape[0]
        print(f'Loading {img_total} images with n_segments = {self.n_segments} ...')
        print(f'Computing features: ')
        self.select_features()

        t = time.time()
        data_list = [self.create_data_obj(d) for d in cifar100]
        t = time.time() - t
        self.loading_time = t
        print(f'Done in {t}s')
...
if __name__ == '__main__' :
    ...
    if args.train:
        ...
        train_ds = SuperPixelGraphCIFAR100(root=args.traindir, 
    ...
    if args.test:
        ...
        test_ds = SuperPixelGraphCIFAR100(root=args.traindir, 
                                       n_segments=args.n_segments,
                                       compactness=args.compactness,
                                       features=args.features,
                                       train=False)
